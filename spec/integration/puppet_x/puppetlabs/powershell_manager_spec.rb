...
describe PuppetX::DscLite::PowerShellManager,
  ...
  describe 'when managing the powershell process' do
    describe 'the PowerShellManager::instance method' do
      it 'returns the same manager instance / process given the same cmd line' do
        ...
        second_pid = manager_a.execute('[Diagnostics.Process]::GetCurrentProcess().Id')[:stdout]

        expect(manager_a).to eq(manager)
  ...
  describe 'when provided powershell commands' do
    ...
    it 'handles writing to stdout and stderr' do
...
try {
  ...
  exit 500
    ...
    def output_cmdlet
      ...
      (current_powershell_major_version >= 3) ? 'Write-Output' : 'Write-Host'
    end

...
describe 'reboot with no invoke, warning' do
  skip 'Implementation of this functionality depends on MODULES-6569' do
    before(:all) do
      ...
      run_shell('reg delete "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager" /v PendingFileRenameOperations /f', accept_all_exit_codes: true)
    end

    after(:all) do
      run_shell("rm -rf /cygdrive/c/#{fake_name}")
    end
    ...
    skip 'Implementation of this functionality depends on MODULES-6569' do
      ...
      context 'MODULES-2843 - when DSC resource requires reboot without "reboot" resource' do
        let(:installed_path) { dsc_resource_fixture_path(:manifest) }

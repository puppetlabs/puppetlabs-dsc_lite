...
describe 'Puppet apply tests' do
  ...
  context 'FM-2623 - Apply DSC Resource Manifest in "noop" Mode Using "puppet apply"' do
    it 'Applies noop manifest' do
      on(windows_agents, puppet('apply --noop --detailed-exitcodes'), :stdin => dsc_manifest_2, :acceptable_exit_codes => [0, 2]) do |result|
        assert_no_match(/Error:/, result.stderr, 'Unexpected error was detected!')
      end
    end
    ...
    it 'Made no changes' do
      ...
      on(windows_agents, "test -f /cygdrive/c/#{test_dir_path_2}/#{fake_name_2}", :acceptable_exit_codes => [1])
    end
  end
